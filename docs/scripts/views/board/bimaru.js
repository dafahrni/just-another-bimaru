...
export class Bimaru {
    constructor() {
        ...
        this.boundLabelSelected = null;
        this.boundTileSelected = null;
        this.cells = [];
        this.labels = [];
        this.editMode = false;
        this.broker.register(MessageType.NewGame, (msg) => this.init(msg));
    }
    init(msg) {
        this.editMode = msg.editMode;
        const size = this.setupHtml(msg.dto.labels);
        this.broker.publish(MessageFactory.sizeChanged(msg.dto, msg.editMode, size));
    }
    setupHtml(labels) {
        ...
        if (!field)
            throw new Error("Field node is missing in HTML.");
        // reset data and remove all tiles and labels
        this.cells.length = 0;
        this.labels.length = 0;
        const childrenToRemove = Array.from(field.children).filter((child) => child.classList.contains("tile") || child.classList.contains("label"));
